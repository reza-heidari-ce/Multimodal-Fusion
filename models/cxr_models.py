...
class CXRModels(nn.Module):
    ...
    def __init__(self, args, device='cpu'):
        ...
        self.args = args
        self.device = device
        if 'vit' in self.args.vision_backbone:
            self.vision_backbone = getattr(torchvision.models, self.args.vision_backbone)(weights=torchvision.models.ViT_B_16_Weights.IMAGENET1K_SWAG_LINEAR_V1)
            cls_layer = self.vision_backbone.heads.head
            d_visual = cls_layer.in_features
            self.vision_backbone.heads.head = nn.Identity(d_visual)
